// WALLY - Wall Plate Customizer v1.1
...
//   Notes:
//
// * The plug types "Duplex Outlet", "Toggle Switch", and "Rocker" will all use the
//   entire plate when selected in top or bottom configuration. The other selection (if
//   not "none" will be selected.
//
...
//Pick a plug or full plate type
2nd_plate = "none";  //	["none":None, "blank":Blank Port, "toggle":Full Plate - Toggle Switch, "outlet":Full Plate - Duplex Outlet, "rocker":Full Plate - Rocker/Designer Plate, "keystone1":Keystone Jack, "vga":VGA Port, "hdmi":HDMI Port, "dvi":DVI-I Port, "displayport":Displayport, "cat5e":Cat5e/Cat6 Port, "usb-a":USB-A Port, "usb-b":USB-B Port, "firewire":Firewire IEEE 1394 Port, "db09":DB-09 Port, "ps2":PS2 Port, "f-type": F-Type/Coaxial Port,"svideo":S-Video Port, "stereo":Stereo Headphone Jack]
//Pick a bottom plug type or leave as none
...
//Pick a plug or full plate type
3rd_plate = "none";  //	["none":None, "blank":Blank Port, "toggle":Full Plate - Toggle Switch, "outlet":Full Plate - Duplex Outlet, "rocker":Full Plate - Rocker/Designer Plate, "keystone1":Keystone Jack, "vga":VGA Port, "hdmi":HDMI Port, "dvi":DVI-I Port, "displayport":Displayport, "cat5e":Cat5e/Cat6 Port, "usb-a":USB-A Port, "usb-b":USB-B Port, "firewire":Firewire IEEE 1394 Port, "db09":DB-09 Port, "ps2":PS2 Port, "f-type": F-Type/Coaxial Port,"svideo":S-Video Port, "stereo":Stereo Headphone Jack]
//Pick a bottom plug type or leave as none
...
//Pick a plug or full plate type
4th_plate = "none";  //	["none":None, "blank":Blank Port, "toggle":Full Plate - Toggle Switch, "outlet":Full Plate - Duplex Outlet, "rocker":Full Plate - Rocker/Designer Plate, "keystone1":Keystone Jack, "vga":VGA Port, "hdmi":HDMI Port, "dvi":DVI-I Port, "displayport":Displayport, "cat5e":Cat5e/Cat6 Port, "usb-a":USB-A Port, "usb-b":USB-B Port, "firewire":Firewire IEEE 1394 Port, "db09":DB-09 Port, "ps2":PS2 Port, "f-type": F-Type/Coaxial Port,"svideo":S-Video Port, "stereo":Stereo Headphone Jack]
//Pick a bottom plug type or leave as none
...
//Pick a plug or full plate type
5th_plate = "none";  //	["none":None, "blank":Blank Port, "toggle":Full Plate - Toggle Switch, "outlet":Full Plate - Duplex Outlet, "rocker":Full Plate - Rocker/Designer Plate, "keystone1":Keystone Jack, "vga":VGA Port, "hdmi":HDMI Port, "dvi":DVI-I Port, "displayport":Displayport, "cat5e":Cat5e/Cat6 Port, "usb-a":USB-A Port, "usb-b":USB-B Port, "firewire":Firewire IEEE 1394 Port, "db09":DB-09 Port, "ps2":PS2 Port, "f-type": F-Type/Coaxial Port,"svideo":S-Video Port, "stereo":Stereo Headphone Jack]
//Pick a bottom plug type or leave as none
...
edgewidth = gang_width[plate_width] + 10; // Bevel setting for top and bottom
rightbevel = gang_width[plate_width] - 4; // Bevel for right side (scales)
...
positions=[height/2,42.9,71.4];
positiona = 42.9; // dual plate upper
positionb = 71.4; // dual plate lower
...
module plate1(){
	if (1st_plate == "toggle" || 1st_plate_bottom_hole == "toggle"){
		translate([0,left_offset,0]) toggle_screws();
		translate([0,left_offset,0]) hole("toggle");
		}
	else if (1st_plate == "rocker" || 1st_plate_bottom_hole == "rocker"){
		translate([0,left_offset,0]) rocker_screws();
		translate([0,left_offset,0]) hole("rocker");
		}
	else if (1st_plate == "outlet" || 1st_plate_bottom_hole == "outlet") {
	translate([0,left_offset,0]) hole("outlet");
	}
	else if (1st_plate_bottom_hole == "none") {
		translate([0,left_offset,0]) box_screws();
		translate([positions[0],left_offset,0]) hole(1st_plate);
			}
	else if (1st_plate == "none") {
		translate([0,left_offset,0]) box_screws();
		translate([positions[0],left_offset,0]) hole(1st_plate_bottom_hole);
			}
	else {
	translate([0,left_offset,0]) box_screws();
	translate([positions[1],left_offset,0]) hole(1st_plate);
	translate([positions[2],left_offset,0]) hole(1st_plate_bottom_hole);
	}
}

module plate2(){
	if (2nd_plate == "toggle" || 2nd_plate_bottom_hole == "toggle"){
		translate([0,left_offset + switch_offset,0]) toggle_screws();
		translate([0,left_offset + switch_offset,0]) hole("toggle");
		}
	else if (2nd_plate == "rocker" || 2nd_plate_bottom_hole == "rocker"){
		translate([0,left_offset + switch_offset,0]) rocker_screws();
		translate([0,left_offset + switch_offset,0]) hole("rocker");
		}
	else if (2nd_plate == "outlet" || 2nd_plate_bottom_hole == "outlet") {
	translate([0,left_offset + switch_offset,0]) hole("outlet");
	}
	else if (2nd_plate_bottom_hole == "none") {
		translate([0,left_offset + switch_offset,0]) box_screws();
		translate([positions[0],left_offset + switch_offset,0]) hole(2nd_plate);
			}
	else if (2nd_plate == "none") {
		translate([0,left_offset + switch_offset,0]) box_screws();
		translate([positions[0],left_offset + switch_offset,0]) hole(2nd_plate_bottom_hole);
			}
	else {
	translate([0,left_offset + switch_offset,0]) box_screws();
	translate([positions[1],left_offset + switch_offset,0]) hole(2nd_plate);
	translate([positions[2],left_offset + switch_offset,0]) hole(2nd_plate_bottom_hole);
	}
}

module plate3(){
	if (3rd_plate == "toggle" || 3rd_plate_bottom_hole == "toggle"){
		translate([0,left_offset + switch_offset * 2,0]) toggle_screws();
		translate([0,left_offset + switch_offset * 2,0]) hole("toggle");
		}
	else if (3rd_plate == "rocker" || 3rd_plate_bottom_hole == "rocker"){
		translate([0,left_offset + switch_offset * 2,0]) rocker_screws();
		translate([0,left_offset + switch_offset * 2,0]) hole("rocker");
		}
	else if (3rd_plate == "outlet" || 3rd_plate_bottom_hole == "outlet") {
	translate([0,left_offset + switch_offset * 2,0]) hole("outlet");
	}
	else if (3rd_plate_bottom_hole == "none") {
		translate([0,left_offset + switch_offset * 2,0]) box_screws();
		translate([positions[0],left_offset + switch_offset * 2,0]) hole(3rd_plate);
			}
	else if (3rd_plate == "none") {
		translate([0,left_offset + switch_offset * 2,0]) box_screws();
		translate([positions[0],left_offset + switch_offset * 2,0]) hole(3rd_plate_bottom_hole);
			}
	else {
	translate([0,left_offset + switch_offset * 2,0]) box_screws();
	translate([positions[1],left_offset + switch_offset * 2,0]) hole(3rd_plate);
	translate([positions[2],left_offset + switch_offset * 2,0]) hole(3rd_plate_bottom_hole);
	}
}

module plate4(){
	if (4th_plate == "toggle" || 4th_plate_bottom_hole == "toggle"){
		translate([0,left_offset + switch_offset * 3,0]) toggle_screws();
		translate([0,left_offset + switch_offset * 3,0]) hole("toggle");
		}
	else if (4th_plate == "rocker" || 4th_plate_bottom_hole == "rocker"){
		translate([0,left_offset + switch_offset * 3,0]) rocker_screws();
		translate([0,left_offset + switch_offset * 3,0]) hole("rocker");
		}
	else if (4th_plate == "outlet" || 4th_plate_bottom_hole == "outlet") {
	translate([0,left_offset + switch_offset * 3,0]) hole("outlet");
	}
	else if (4th_plate_bottom_hole == "none") {
		translate([0,left_offset + switch_offset * 3,0]) box_screws();
		translate([positions[0],left_offset + switch_offset * 3,0]) hole(4th_plate);
			}
	else if (4th_plate == "none") {
		translate([0,left_offset + switch_offset * 3,0]) box_screws();
		translate([positions[0],left_offset + switch_offset * 3,0]) hole(4th_plate_bottom_hole);
			}
	else {
	translate([0,left_offset + switch_offset * 3,0]) box_screws();
	translate([positions[1],left_offset + switch_offset * 3,0]) hole(4th_plate);
	translate([positions[2],left_offset + switch_offset * 3,0]) hole(4th_plate_bottom_hole);
	}
}

module plate5(){
	if (5th_plate == "toggle" || 5th_plate_bottom_hole == "toggle"){
		translate([0,left_offset + switch_offset * 4,0]) toggle_screws();
		translate([0,left_offset + switch_offset * 4,0]) hole("toggle");
		}
	else if (5th_plate == "rocker" || 5th_plate_bottom_hole == "rocker"){
		translate([0,left_offset + switch_offset * 4,0]) rocker_screws();
		translate([0,left_offset + switch_offset * 4,0]) hole("rocker");
		}
	else if (5th_plate == "outlet" || 5th_plate_bottom_hole == "outlet") {
	translate([0,left_offset + switch_offset * 4,0]) hole("outlet");
	}
	else if (5th_plate_bottom_hole == "none") {
		translate([0,left_offset + switch_offset * 4,0]) box_screws();
		translate([positions[0],left_offset + switch_offset * 4,0]) hole(5th_plate);
			}
	else if (5th_plate == "none") {
		translate([0,left_offset + switch_offset * 4,0]) box_screws();
		translate([positions[0],left_offset + switch_offset * 4,0]) hole(5th_plate_bottom_hole);
			}
	else {
	translate([0,left_offset + switch_offset * 4,0]) box_screws();
	translate([positions[1],left_offset + switch_offset * 4,0]) hole(5th_plate);
	translate([positions[2],left_offset + switch_offset * 4,0]) hole(5th_plate_bottom_hole);
	}
}
...
if (1st_plate_bottom_hole == "keystone1" && 1st_plate == "none") {
	...
	translate([85.6,23.4,-3.9]) hole("keystone_solid");
	}
...
if (2nd_plate == "keystone1" && 2nd_plate_bottom_hole == "none") {
	translate([71.4,23.4 + switch_offset,-3.9]) hole("keystone_solid");
	}
else if (2nd_plate == "keystone1" && 2nd_plate_bottom_hole != "outlet" && 2nd_plate_bottom_hole != "toggle" && 2nd_plate_bottom_hole != "rocker") {
	translate([57.2,23.4 + switch_offset,-3.9]) hole("keystone_solid");
	}
if (2nd_plate_bottom_hole == "keystone1" && 2nd_plate == "none") {
	...
	translate([85.6,23.4 + switch_offset,-3.9]) hole("keystone_solid");
	}
...
if (3rd_plate == "keystone1" && 3rd_plate_bottom_hole == "none") {
	translate([71.4,23.4 + switch_offset * 2,-3.9]) hole("keystone_solid");
	}
else if (3rd_plate == "keystone1" && 3rd_plate_bottom_hole != "outlet" && 3rd_plate_bottom_hole != "toggle" && 3rd_plate_bottom_hole != "rocker") {
	translate([57.2,23.4 + switch_offset * 2,-3.9]) hole("keystone_solid");
	}
if (3rd_plate_bottom_hole == "keystone1" && 3rd_plate == "none") {
	...
	translate([85.6,23.4 + switch_offset * 2,-3.9]) hole("keystone_solid");
	}
...
if (4th_plate == "keystone1" && 4th_plate_bottom_hole == "none") {
	translate([71.4,23.4 + switch_offset * 3,-3.9]) hole("keystone_solid");
	}
else if (4th_plate == "keystone1" && 4th_plate_bottom_hole != "outlet" && 4th_plate_bottom_hole != "toggle" && 4th_plate_bottom_hole != "rocker") {
	translate([57.2,23.4 + switch_offset * 3,-3.9]) hole("keystone_solid");
	}
if (4th_plate_bottom_hole == "keystone1" && 4th_plate == "none") {
	...
	translate([85.6,23.4 + switch_offset * 3,-3.9]) hole("keystone_solid");
	}
...
if (5th_plate == "keystone1" && 5th_plate_bottom_hole == "none") {
	translate([71.4,23.4 + switch_offset * 4,-3.9]) hole("keystone_solid");
	}
else if (5th_plate == "keystone1" && 5th_plate_bottom_hole != "outlet" && 5th_plate_bottom_hole != "toggle" && 5th_plate_bottom_hole != "rocker") {
	translate([57.2,23.4 + switch_offset * 4,-3.9]) hole("keystone_solid");
	}
if (5th_plate_bottom_hole == "keystone1" && 5th_plate == "none") {
	...
	translate([85.6,23.4 + switch_offset * 4,-3.9]) hole("keystone_solid");
	}
...
module plate() {
	difference() {
		cube([height,gang_width[plate_width],6]);
		translate([-4.3,-5,6.2]) rotate([0,45,0]) cube([6,edgewidth,6]); //Top Bevel
		translate([110.1,-5,6.25]) rotate([0,45,0]) cube([6,edgewidth,6]); //Bottom Bevel
		translate([120,-4.4,6.1]) rotate([0,45,90]) cube([6,124,6]); //Left Bevel (doesn't change)
		translate([120,rightbevel,6]) rotate([0,45,90]) cube([6,124,6]); //Right Bevel (scales right)
				}
			}
...
module plate_inner() {
	...
	translate([3,3,0]){
	difference() {
		cube([114.3,gang_width[plate_width],6.5]);
		translate([-4.3,-5,6.2]) rotate([0,45,0]) cube([6,edgewidth,6]); //Top Bevel
		translate([110.1,-5,6.25]) rotate([0,45,0]) cube([6,edgewidth,6]); //Bottom Bevel
		translate([120,-4.4,6.1]) rotate([0,45,90]) cube([6,124,6]); //Left Bevel (doesn't change)
		translate([120,rightbevel,6]) rotate([0,45,90]) cube([6,124,6]); //Right Bevel (scales right)
...
// Box screw holes
module box_screws(){
	 translate([15.4964,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([15.4964,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
	 translate([98.8389,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([98.8389,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
}

// Rocker/Designer screw holes
module rocker_screws(){
	 translate([8.7964,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([8.7964,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
	 translate([105.5389,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([105.5389,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
}

// Toggle screw holes
module toggle_screws(){
	 translate([26.987,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([26.987,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
	 translate([87.313,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([87.313,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
}
...
// Toggle switch hole and screw holes
	if (hole_type == "toggle") {
		translate([57.15,0,0]) cube([23.8125,10.3188,15], center = true);
		 						}

// Rocker switch plate
	if (hole_type == "rocker") {
		translate([57.15,0,0]) cube([67.1,33.3,15], center = true);

		 						}

// Duplex power outlet plate or dual side toggles
	if (hole_type == "outlet" || hole_type == "dualsidetoggle") {
		translate([37.820,0,0]) {
			difference() {
		...
		translate([57.15,0,-1]) cylinder(r=2, h=10);
		translate([57.15,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
							}
...
// Fits http://www.datapro.net/products/f-type-panel-mounting-coupler.html
	if (hole_type == "f-type") {
		translate([0,0,3]) cylinder(r=5.555, h=10, center=true);
							}
...
translate([-57.15,-gang_width[plate_width]/2,-6]){
